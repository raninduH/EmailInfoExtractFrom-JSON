...
# import sys
# sys.path.append('..') 
from email_filtering_and_info_generation.configurations.database import collection_email_msgs
from email_filtering_and_info_generation.configurations.database import collection_triger_events,collection_trigers,collection_readingEmailAccounts,collection_suggestions, collection_conversations, collection_issues, collection_inquiries, collection_overdue_trigger_events, collection_configurations
from email_filtering_and_info_generation.models import Convo_summary, Email_msg, InquiryInDB, IssueInDB, Overdue_trig_event,Trigger_event,Reading_email_acc,Suggestion
from email_filtering_and_info_generation.schemas import individual_email_msg_serial,list_email_msg_serial
from email_filtering_and_info_generation.schemas import individual_trigger_serial,list_trigger_serial
from email_filtering_and_info_generation.schemas import individual_readingEmailAcc_serial, list_readingEmailAcc_serial
...
async def update_summary(thread_id: str, new_summary: str, new_updated_times:List[datetime]):
    ...
    if result.matched_count == 0:
        ...
        print("Already existing summary updated")
...
async def update_inquiry_status(thread_id_to_update:str, new_inquiry_convo_summary_arr:str, updated_time:datetime, new_status:str, new_ongoing_status:str, sentiment_score: float,end_time_value:datetime, new_effectiveness:str, new_efficiency:str):
    try:
        # Update the document with the specified thread_id
        result = collection_inquiries.update_one(
            {"thread_id": thread_id_to_update},
            {"$set": {"inquiry_convo_summary_arr":new_inquiry_convo_summary_arr, "updated_time":updated_time, 
                      "status": new_status, "ongoing_status":new_ongoing_status, "sentiment_score":sentiment_score,
                      "end_time": end_time_value, "effectiveness":new_effectiveness, "efficiency":new_efficiency}}
        )
        
        print("Inquiry in DB updated")

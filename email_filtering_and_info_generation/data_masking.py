from email_filtering_and_info_generation.services import get_all_reading_accounts, getProductsList
...
def create_entity_map(model_output, unmasked_text, products_words):
    ...
    for token in model_output:
        ...
        if token["entity_group"] not in ['USERAGENT','CREDITCARDISSUER', 'STATE', 'URL', 'BUILDINGNUMBER', 'DATE', 'TIME', 'JOBTYPE', 'JOBAREA', 'VEHICLEVRM', 'CURRENCYCODE', 'AGE','JOBTITLE', 'SSN', 'SECONDARYADDRESS','COMPANYNAME','PIN','CREDITCARDCVV'] and entity not in products_words:
